...
def evaluate_request(code,values,profile=True,budget=1_000_000_000):
    return dict(protocol=1,sigversion='tapscript_v2',script=code.hex(),stack=[v.hex() for v in values],varops_budget=budget,profile=profile)
...
def timing(command,request,repeats=11):
    ...
    for _ in range(repeats):
        ...
        assert a[key]==b[key]
        observations.append(a);plain.append(b)
    return dict(observed_interpreter_ns=distribution([x['profile']['interpreter_ns'] for x in observations]),
                unobserved_interpreter_ns=distribution([x['profile']['interpreter_ns'] for x in plain]),
                cli_wall_ns=distribution([x['wall_ns'] for x in plain]),
                process_and_protocol_overhead_ns=distribution([x['process_and_protocol_overhead_ns'] for x in plain]))

def main():
    vectors=json.loads((ROOT/'fixtures/vectors.json').read_text())['vectors']
    data=dict(standalone=[],transactions=[],rejected_transactions=[],
              caveats=['Timing excludes process startup/JSON parsing from interpreter_ns.',
    ...
    for profile in ('baseline','bytes','full'):
        for mode in ('unified','stateful','stateless'):
            ...
            for v in selected:
                ...
                if mode!='unified' and (mode=='stateless')!=is_sl:continue
                values=decode(v)
                request=evaluate_request(program.code,values)
                native=evaluate(program.code,values)
                observed=run_profile('evalscript',request)
                assert native.success and observed['success']
                assert native.remaining==observed['varops-budget-remaining']
                assert observed['stack-after']==[]
                if not is_sl:
                    depth=255-values[0][0]
                    assert observed['profile']['sha256_calls']==244+depth
                bad=[values[0],values[1],bytes([values[2][0]^1])+values[2][1:]]
                native_bad=evaluate(program.code,bad)
                observed_bad=run_profile('evalscript',evaluate_request(program.code,bad))
                assert not native_bad.success and not observed_bad['success']
                assert native_bad.remaining==observed_bad['varops-budget-remaining']
                altered_path=values[0][:-1]+bytes([values[0][-1]^1])
                late=evaluate(program.code,[altered_path,*values[1:]])
                late_profile=run_profile('evalscript',evaluate_request(program.code,[altered_path,*values[1:]]))
                assert not late.success and not late_profile['success']
                assert late.remaining==late_profile['varops-budget-remaining']
                malformed=run_profile('evalscript',evaluate_request(program.code,[values[0][:-1],*values[1:]]))
                assert not malformed['success']
                data['standalone'].append(dict(profile=profile,mode=mode,fixture=v['name'],
                    script_bytes=len(program.code),script_sha256=hashlib.sha256(program.code).hexdigest(),
                    input_sha256=hashlib.sha256(b''.join(values)).hexdigest(),signature_bytes=len(values[0]),
                    varops_consumed=native.consumed,metrics=observed['profile'],
                    functions={name:fid for name,(fid,_,_) in program.functions.items()},
                    rejected_message_varops=native_bad.consumed,rejected_message_metrics=observed_bad['profile'],
                    rejected_authentication_path_varops=late.consumed,rejected_authentication_path_metrics=late_profile['profile'],
                    malformed_length_varops=1_000_000_000-malformed['varops-budget-remaining'],
    ...
    for row in records['spends']:
        ...
        assert result['success'],(row['name'],result)
        assert result['varops_allowed']==row['varops_allowed']==10_000*row['weight']
        assert result['weight']==row['weight']
        assert result['varops_consumed']==sum(x['consumed'] for x in result['inputs'])
        assert result['varops_consumed']<=result['varops_allowed']
        record={k:v for k,v in row.items() if k not in ('raw_transaction','spent_outputs','transcript')}
        record.update(varops_consumed=result['varops_consumed'],budget_fraction=result['varops_consumed']/result['varops_allowed'],
                      metrics=result['profile'],inputs=result['inputs'],timing=timing('measuretx',request),padding_bytes=0)
        data['transactions'].append(record)
    for row in records['rejections']:
        request=dict(transaction=row['raw_transaction'],spent_outputs=row['spent_outputs'])
        result=run_profile('measuretx',request)
        assert not result['success'],row['name']
        assert not any('budget' in x.get('error','').lower() for x in result['inputs']),row['name']
        data['rejected_transactions'].append(dict(name=row['name'],node_result=row['result'],
              varops_consumed=result['varops_consumed'],varops_allowed=result['varops_allowed'],
              failure=result['inputs'],metrics=result['profile']))
    data['empty_program_cli_baseline']=timing('evalscript',evaluate_request(b'\x51',[]),21)
    data['recorded_transactions_sha256']=hashlib.sha256((ROOT/'reports/regtest-details.json.gz').read_bytes()).hexdigest()
    ...
    for row in data['transactions']:
        lines.append(f"| {row['name']} | {row.get('program_bytes','two scripts')} | {row.get('signature_bytes','two signatures')} | {row['weight']:,} | {row['vsize']:,} | {row['varops_consumed']:,} / {row['varops_allowed']:,} | {row['budget_fraction']:.1%} |")
    lines += ['', 'The public key is 48 bytes, embedded in each policy. Single-input rows use a 65-byte control block (two committed leaves); the mixed-input rows use a 33-byte control block per input. The latter use a separate all-input/all-output policy with an explicit four-input cap. Mode-specific leaves are paired with their opposite mode in one tree; combined leaves have an extra mode-specific sibling for cross-policy replay tests.',
              ...
              '|---|---:|---:|---:|---:|---:|']
    for row in data['transactions']:
        m=row['metrics'];t=row['timing']['unobserved_interpreter_ns']['median']/1e6
        lines.append(f"| {row['name']} | {m['peak_stack_bytes']:,} | {m['peak_total_bytes']:,} | {m['max_item_bytes']:,} | {m['executed_function_body_bytes']:,} | {t:.3f} |")
    lines += ['', f"All {len(data['rejected_transactions'])} recorded transaction mutations also fail the offline transaction-aware Script checker, including annex and oversized two-input cases that relay policy rejects first. This distinguishes cryptographic/policy rejection from process failure or insufficient budget.",
              ...
              '', '## Measurement boundaries','']+['- '+x for x in data['caveats']]
    (ROOT/'reports/RESULTS.md').write_text('\n'.join(lines)+'\n')
    print(f"Measured {len(data['standalone'])} standalone classes, {len(data['transactions'])} spends and {len(data['rejected_transactions'])} rejected transactions")
