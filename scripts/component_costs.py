...
def main():
    rows=[];active={};test_name=''
    old_component=suite.component
    def component(inputs,build,values,expected,profile):
        active.update(profile=profile,inputs=list(inputs),expected_width=len(expected))
        try:return old_component(inputs,build,values,expected,profile)
        finally:active.clear()
    def observed(code,stack=(),budget=1_000_000_000,**kwargs):
        result=evaluate(code,stack,budget,**kwargs)
        if active:
            rows.append(dict(active,test=test_name,script_bytes=len(code),
                ...
                accepted=result.success,varops_consumed=result.consumed,cli_seconds=result.elapsed_seconds))
        return result
    class Result(unittest.TextTestResult):
        def startTest(self,test):
            nonlocal test_name
            test_name=test.id();super().startTest(test)
    suite.component=component;suite.evaluate=observed
    tests=unittest.TestSuite(unittest.defaultTestLoader.loadTestsFromTestCase(cls)
          for cls in (suite.Components,suite.HashAndWotsAgreement,suite.StatelessComponents))
    result=unittest.TextTestRunner(resultclass=Result,verbosity=1).run(tests)
    if not result.wasSuccessful():raise SystemExit(1)
    assert rows and all(row['accepted'] for row in rows)
    data=dict(measurement_scope='Component agreement harness including expected-output comparison and cleanup',
