...
def main():
    ...
    for depth in range(1, 256):
        for index in (0, 2 ** min(depth, 64) - 1):
            v = synthetic_stateful(depth, index)
            values = decode(v)
            for profile, p in programs.items():
                r = evaluate(p.code, values)
                m = run_profile("evalscript", evaluate_request(p.code, values))
                require(r.success and m["success"])
                require(r.remaining == m["varops-budget-remaining"])
                require(m["profile"]["sha256_calls"] == 244 + depth)
                rows.append(
                    dict(
                        depth=depth,
                        index=index,
                        ...
                        script_bytes=len(p.code),
                        script_sha256=hashlib.sha256(p.code).hexdigest(),
                        input_sha256=hashlib.sha256(b"".join(values)).hexdigest(),
                        varops_consumed=r.consumed,
                        metrics=m["profile"],
                    )
                )
    (ROOT / "reports/boundary-costs.json.gz").write_bytes(
        gzip.compress(json.dumps(rows, sort_keys=True).encode(), mtime=0)
    )
    summary = dict(
        rows=len(rows),
        depths=255,
        index_classes=["zero", "maximum"],
        fixture_provenance="Synthetic authentication siblings with genuine WOTS signing and computed root binding",
        allowance="Standalone functional allowance; transaction feasibility is measured separately in costs.json",
        profiles={},
    )
    for profile in programs:
        subset = [x for x in rows if x["profile"] == profile]
        summary["profiles"][profile] = dict(
            varops_min=min(x["varops_consumed"] for x in subset),
            ...
            maximum_invoked_body_bytes=max(
                x["metrics"]["executed_function_body_bytes"] for x in subset
            ),
        )
    summary["details_sha256"] = hashlib.sha256(
        (ROOT / "reports/boundary-costs.json.gz").read_bytes()
    ).hexdigest()
    (ROOT / "reports/boundary-costs.json").write_text(
        json.dumps(summary, indent=2) + "\n"
    )
    print(
        f"Measured {len(rows)} boundary executions with matching instrumented/native results"
    )
