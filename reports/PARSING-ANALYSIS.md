...
The current [counts](parsing-counts.json) and [parser timings](parser-benchmark.json) are regenerated by the complete check pipeline.
Their provenance identifies the current transaction corpus, cost report, measurement code, and executables.
The audit checks those links and reconstructs every parsing schedule.
The parser and interpreter reports now identify their measurement environment directly.

The tables below preserve the initial investigation at commit `490242c`.
They are historical observations, not the current timing table.
Their corpus SHA256 is `d45c2171cca76e0b02922bf37e41e9c99dcd384f39f26905e8bf2f46be726702`.
The [baseline tag](https://github.com/otaliptus/gsr-shrincs/blob/baseline-2026-09-18/reports/regtest-details.json.gz) preserves that corpus.
...
Historical evidence: [pinned parsing counts](https://github.com/otaliptus/gsr-shrincs/blob/490242cd18dad38c5d81c6e80d8e520e6b2fb52f/reports/parsing-counts.json).
...
Historical evidence: [pinned parser benchmark](https://github.com/otaliptus/gsr-shrincs/blob/490242cd18dad38c5d81c6e80d8e520e6b2fb52f/reports/parser-benchmark.json).
This includes source hashes, executable hashes, counts, and raw samples.
...
This builds a laboratory command under `build/parser-benchmark`.
It preserves the counting executable under `build/parsing-probe`. It does not change Script execution or charging rules.

Use `python3 scripts/audit_followups.py --portable` to check committed follow-up provenance on another machine.
This checks recorded relationships and current inputs. It does not verify local executables or reproduce timings.
The full evidence audit checks the separate local builds as well.
