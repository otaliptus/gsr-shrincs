...
def deterministic(label, length):
    return hashlib.shake_256(('PUBLIC GSR SHRINCS TEST ONLY:'+label).encode()).digest(length)
...
def fixture(name, message, signature, public_key, provenance, context=CONTEXT):
    assert verify(message,signature,public_key,context), name
    return dict(name=name,message=message.hex(),signature=signature.hex(),
                public_key=public_key.hex(),context=context.hex(),provenance=provenance)

def decode(vector):
    return tuple(bytes.fromhex(vector[k]) for k in ('signature','public_key','message'))

def synthetic_stateful(depth, index, label='boundary'):
    ...
    for k in range(depth):
        ...
        ad=bytearray(bytes([height+k+1])+(index>>(k+1)).to_bytes(8,'big')+b'\x12'+bytes(12))
        node=scheme.H(seed,ad,sibling+node if (index>>k)&1 else node+sibling)
    sf=node
    ...
    assert wots is not None
    sig=bytes([height])+randomizer+index.to_bytes((min(depth,64)+7)//8,'big')+wots+siblings
    return fixture(f'synthetic-depth-{depth}-index-{index}',message,sig,seed+sl+sf,
                   dict(kind='synthetic-authentication-path',public_seed_label=label,
                        depth=depth,index=index,full_tree_generated=False))
...
def real_vectors():
    ...
    for state in (0,1,7,15,None):
        ...
        result.append(fixture(f'genuine-{state}',message,signature,pk,
                      dict(kind='reference-keygen-sign-verify',public_seed=PUBLIC_SEED.hex(),
                           structure='0104',state_counter=state)))
    ...
    for state in (0,1,3,4,None):
        ...
        signature=scheme.shrincs_sign(message,CONTEXT,key,state,None)
        result.append(fixture(f'genuine-unbalanced-{state}',message,signature,pk,
                      dict(kind='reference-keygen-sign-verify',public_seed=second.hex(),
                           structure='0004',state_counter=state)))
    return result
