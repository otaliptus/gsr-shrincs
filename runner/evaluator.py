...
DEFAULT_BINARY = Path(
    os.environ.get("GSR_EVAL_BINARY", ROOT / "build/bitcoin/bin/bitcoin-util")
)
...
def evaluate(
    script, stack=(), budget=1_000_000_000, *, binary=DEFAULT_BINARY, timeout=30
):
    ...
    request = dict(
        ...
        script=script.hex(),
        stack=[bytes(x).hex() for x in stack],
        varops_budget=budget,
    )
    start = time.perf_counter()
    try:
        p = subprocess.run(
            [str(binary), "evalscript"],
    ...
    try:
        ...
        require(
            type(data["protocol"]) is int
            and data["protocol"] == 1
            and data["context"] == "standalone"
        )
        require(data["sigversion"] == "tapscript_v2")
        require(type(data["success"]) is bool)
        require(
            (data["error"] is None)
            if data["success"]
            else isinstance(data["error"], str)
        )
        remaining = data["varops-budget-remaining"]
        require(type(remaining) is int and 0 <= remaining <= budget)
        require(type(data["stack-after"]) is list)
    ...
    return Result(
        data["success"], data["error"], final, remaining, budget - remaining, elapsed
    )
