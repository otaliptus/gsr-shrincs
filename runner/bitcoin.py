...
def transcript(tx, spent_outputs, input_index, script, annex=b'', codesep=0xffffffff):
    if len(spent_outputs)!=len(tx.vin):raise ValueError('missing spent outputs')
...
def message(tx,spent_outputs,input_index,script,**kwargs):
    return hashlib.sha256(transcript(tx,spent_outputs,input_index,script,**kwargs)).digest()
