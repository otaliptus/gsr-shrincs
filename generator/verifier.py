...
def h_grind(pad, location, digest, counter_bytes):
    return op(
        "LEFT", sha(cat(pad, location, b"\x16", digest, bytes(4), counter_bytes)), 16
    )
...
def h_msg_sf(randomizer, seed, root, location, bound):
    return sha(
        cat(
            randomizer,
            seed,
            sha(cat(randomizer, seed, root, location, bound)),
            location,
        )
    )
...
def chain_body(b):
    ...
    for j in range(15):
        b.branch(
            op("LESSTHANOREQUAL", R("digit"), j),
            lambda b, j=j: b.let(
                "node",
                hash16(R("pad"), cat(R("prefix"), j.to_bytes(4, "big")), R("node")),
            ),
        )
    b.finish(R("node"))
...
def wots_body(b, constant_sum):
    ...
    if constant_sum:
        ...
        for shift in (8, 4, 0):
            b.let(
                "digits", cat(R("digits"), be(b, mod(shr(R("checksum"), shift), 16), 1))
            )
    b.let("tips", b"")
    for i in range(32 if constant_sum else 35):
        b.call(
            ...
            chain_body,
            (
                cut(R("sig"), (2 if constant_sum else 0) + 16 * i, 16),
                cut(R("digits"), i, 1),
                cat(
                    R("location"),
                    bytes([16 if constant_sum else 0]),
                    R("keypair"),
                    i.to_bytes(4, "big"),
                ),
                R("pad"),
    ...
    b.finish(
        hash16(
            ...
            cat(
                R("location"),
                bytes([17 if constant_sum else 1]),
                R("keypair"),
                bytes(8),
            ),
            R("tips"),
...
def wots(b, sig, msg, location, keypair, constant_sum, output):
    return b.call(
        "wots_c" if constant_sum else "wots_tw",
        WOTS_ARGS,
        lambda b: wots_body(b, constant_sum),
        (sig, msg, R("pad"), location, keypair),
        output,
    )
...
def pair(b, node, sibling, index, address, output):
    return b.call(
        "merkle_pair",
        ("node", "sibling", "index", "address", "pad"),
        pair_body,
        (node, sibling, index, address, R("pad")),
        output,
    )


AUTH_ARGS = ("node", "path", "index", "height", "depth", "pad")


def authentication_block(b, level):
    """Process a nonempty path of at most 2**level siblings without recursion.

    A block splits its path between two lower-level functions. Passing only the
    child's path keeps deep-tree copying costs bounded as well as bytecode size.
    """
    b.mark(f"authentication_block_{level}")
    if level == 0:
        address = cat(
            be(b, add(R("height"), 1), 1),
            be(b, shr(R("index"), 1), 8),
            b"\x12",
            bytes(12),
        )
        pair(b, R("node"), R("path"), R("index"), address, "node")
    else:
        half = 2 ** (level - 1)
        left_depth = op("MIN", R("depth"), half)
        args = (
            R("node"),
            cut(R("path"), 0, op("MUL", left_depth, 16)),
            R("index"),
            R("height"),
            left_depth,
            R("pad"),
        ...
        b.call(
            f"authentication_{level - 1}",
            AUTH_ARGS,
            lambda child: authentication_block(child, level - 1),
            args,
            "node",
        )

        def right(block):
            remaining = sub(R("depth"), half)
            args = (
                R("node"),
                cut(R("path"), half * 16, op("MUL", remaining, 16)),
                shr(R("index"), half),
                add(R("height"), half),
                remaining,
                R("pad"),
        ...
        b.branch(op("LESSTHAN", half, R("depth")), right)
    b.finish(R("node"))
...
def stateful_body(b):
    ...
    b.let(
        "index_be",
        cat(op("LEFT", bytes(8), sub(8, R("width"))), cut(R("sig"), 17, R("width"))),
    )
    ...
    b.let(
        "digest",
        h_msg_sf(
            R("randomizer"), R("seed"), cut(R("pk"), 32, 16), R("location"), R("bound")
        ),
    )
    wots(
        b,
        cut(R("sig"), add(17, R("width")), 514),
        R("digest"),
        R("location"),
        bytes(4),
        True,
    ...
    if b.profile == "full" and not b.library.inline:
        args = (
            R("node"),
            cut(R("sig"), add(531, R("width")), op("MUL", R("depth"), 16)),
            R("index"),
            R("height"),
            R("depth"),
            R("pad"),
        ...
        b.call(
            "authentication_8",
            AUTH_ARGS,
            lambda block: authentication_block(block, 8),
            args,
            "node",
        )
    else:
        # Preserve the independently expanded implementation for comparison.
        for k in range(255):
            ...
            def step(b, k=k):
                pair(
                    b,
                    R("node"),
                    cut(R("sig"), add(add(531, R("width")), 16 * k), 16),
                    shr(R("index"), k),
                    cat(
                        be(b, add(R("height"), k + 1), 1),
                        be(b, shr(R("index"), k + 1), 8),
                        b"\x12",
                        bytes(12),
                    ),
                    "node",
...
def fors_body(b):
    ...
    for i in range(10):
        b.let(
            "index", add(i * 8192, mod(shr(R("digest_num"), 136 - 13 * (i + 1)), 8192))
        )
        b.let(
            "node",
            hash16(
                R("pad"),
                cat(
                    R("location"), b"\x03", R("keypair"), bytes(4), be(b, R("index"), 4)
                ),
                cut(R("sig"), i * 224, 16),
            ),
        )
    ...
    b.finish(
        hash16(
            R("pad"), cat(R("location"), b"\x04", R("keypair"), bytes(8)), R("roots")
        )
    )
...
def stateless_body(b):
    ...
    b.let(
        "digest", h_msg_sl(R("randomizer"), R("seed"), cut(R("pk"), 16, 16), R("bound"))
    )
    ...
    for layer in range(5):
        ...
        for k in range(9):
            pair(
                b,
                R("node"),
                cut(R("sig"), 2257 + 704 * layer + 560 + 16 * k, 16),
                shr(R("leaf"), k),
                cat(
                    R("location"),
                    b"\x02",
...
def compile_verifier(profile="full", mode="unified", context=CONTEXT):
    if profile not in ("full", "baseline", "bytes") or mode not in (
        "unified",
        "stateful",
        "stateless",
    ):
    ...
    b = Builder(
        ("sig", "pk", "msg"), "baseline" if profile == "baseline" else "full", lib
    )
    ...
    def sf(b):
        b.call("stateful", MODE_ARGS, stateful_body, args, "valid")

    def sl(b):
        b.call("stateless", MODE_ARGS, stateless_body, args, "valid")
