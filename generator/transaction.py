...
def compile_policy(public_key, context=CONTEXT, mode='unified', max_inputs=1, profile='full'):
    ...
    b=Builder(('sig',),'full')
    b.emit('DEPTH');b.stack.append(None)
    b.literal(1);b.emit('NUMEQUAL');b.stack.pop();b.emit('VERIFY');b.stack.pop()
    ...
    assert b.stack==['sig','pk','msg']
    verifier=compile_verifier(profile,mode,context)
    program=Program(bytes(b.code)+verifier.code,'full',mode,
